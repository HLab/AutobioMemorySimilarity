...
// var mycondition = 0; // hardcode the condition because we want to bypass psiTurk's flawed counterbalancing mechanism
...
var pages = [
	...
	"memory_experiment_describe_later.html"
];
...
var memoryQuestionnaire = function() {
  ...
  *****************************/
	...
	function textSubmitted() {
		if ((questionID == 'describe_memoryA') || (questionID == 'describe_memoryB')) {
		...
		} else if (questionID == 'relateAB') {
			if ($('#' + 'responseID_' + questionID).val().length < minCountRelate) {
				raiseError = true;
				$('#count_' + questionID).text('Please enter at least one sentence.');
			} else {
				var mem = questionID.slice(-2, -1);
				psiTurk.recordUnstructuredData('keyPressesMem' + mem, keyPresses['responseID_' + questionID]);
				raiseError = false;
	...
	function nextTrial() {
		...
		if (phase > nPhases) {
			...
			else if (questionID == 'retrievalType') {
				if (response == '1') {
					// remove memory B prompt; we ask for the one they triggered directly.
					questionList[0].splice(questionList[0].indexOf('prompt_memoryB'), 1);
					nTrials[0]--; // total number of trials is reduced by 1
					$('#placeholder_memoryB .innerDiv').html('Great! We are going to call this triggered memory <span style="color: blue">Memory B</span>.<br><br>Take a moment to place yourself back in the situation of <span style="color: blue">Memory B</span>. Make your memory as specific as possible.');
					$('#relateAB .innerDiv').html(htmlByCondition[0]['relateAB']);
				}
				else {
					$('#placeholder_memoryB .innerDiv').html('Great! We are going to call your second memory <span style="color: blue">Memory B</span>.<br><br>Take a moment to place yourself back in the situation of <span style="color: blue">Memory B</span>. Make your memory as specific as possible.');
					$('#relateAB .innerDiv').html(htmlByCondition[mycondition]['relateAB']);
				}
			}
			else if (questionID == 'describe_memoryA') {
				$('#response_memoryA').hide();
				document.getElementById('responseA').innerHTML = $('#describe_memoryA_store').val();
				$('#responseA').show()
			}
			else if (questionID == 'describe_memoryB') {
				$('#response_memoryB').hide();
				document.getElementById('responseB').innerHTML = $('#describe_memoryB_store').val();
				$('#responseB').show();
			}

			// Update questionID
			questionID = questionList[phase-1][curTrial-1];
			if (questionID == 'placeholder_memoryA') {
				$('#memoryA').css('border-style', 'solid');
				$('#memoryA').css('border-width', '2px');
				$('#titleA').show();
			}
			else if (questionID == 'placeholder_memoryB') {
				$('#memoryB').css('border-style', 'solid');
				$('#memoryB').css('border-width', '2px');
				$('#titleB').show();
			}
			else if (questionID == 'describe_memoryA') {
				$('#response_memoryA').show();
			}
			else if (questionID == 'describe_memoryB') {
				$('#response_memoryB').show();
			}
			$('#' + questionID).show();
			startTime = new Date().getTime();
		}
  ...
  ***************************/
  ...
  psiTurk.showPage('memory_experiment_describe_later.html');

	/* define the condition-dependent html */
	var htmlByCondition = {
													0:{
														'prompt_memoryB':'Think about <span style="color: green">Memory A</span>.<br><br>Does it <em>remind you</em> of any other <em>specific</em> event from your life?<br><br>Take a moment to think of this other memory.<br><br>Click to continue once you have your second memory in mind.',
														'relateAB':'Why did <span style="color: green">Memory A</span> remind you of <span style="color: blue">Memory B</span>?',
														},
													1:{
														'prompt_memoryB':'Think about <span style="color: green">Memory A</span>.<br><br>Can you think of any other <em>specific</em> event from your life that is <em>very dissimilar</em> from the one you just recalled?<br><br>Take a moment to think of this other memory.<br><br>Click to continue once you have your second memory in mind.',
														'relateAB':'What makes <span style="color: green">Memory A</span> and <span style="color: blue">Memory B</span> different from one another?',
														}
												};

	/* populate condition-dependent DOMs */
	$('#prompt_memoryB .innerDiv').html(htmlByCondition[mycondition]['prompt_memoryB']);
  ...
  /* shuffle catch trial order */
	...
	questionList[0].splice(questionList[0].indexOf('describe_memoryA'), 2);
	...
	/* insert memory description trials after everything else in phase 1 */
questionList[0].push('describe_memoryA');
questionList[0].push('describe_memoryB');
  ...
  $('.emotionFinish').click(submitEmotionRatings);
	...
	$('#responseID_describe_memoryA').keyup(recordKeyPresses);
	$('#responseID_describe_memoryB').keyup(recordKeyPresses);
